...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await db.from("placements").insert(clean).select("*").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await writeAudit(db, { user_id: auth.profile.id, action: "create", entity_type: "placement", entity_id: data.id, new_value: { name: data.name } });
